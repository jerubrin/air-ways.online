export enum LocalStorageKeys {
  Currency = 'air-ways.online_currency',
  Cart = 'air-ways.online_cart',
  CartIndex = 'air-ways.online_cart-index',

  Flights = 'air-ways.online_store_flights',
  PassengersResult = 'air-ways.online_store_passengers-result',
  RandomData = 'air-ways.online_store_random-data',
  FlightResults = 'air-ways.online_store_flight-results',
  QueryParams = 'air-ways.online_store_query-params',
  SelectedFlights = 'air-ways.online_store_selected-flights',

  IsCartData = 'air-ways.online_is_cart_data',

  Token = 'air-ways.online_token',
  UserName = 'air-ways.online_user-name',
  UserData = 'air-ways.online_user-data'
}
